...
def Candidates(varyingData,constantData):
    ...
    version:      Mean number of Ar39 photons per microsecond
    ...
    [events,eventsSN,trigDur,resolution,version,time_and_PD,simulationTime] = constantData
    ...
    # Threshold will be give in photons
        ...
        if(photons_in_region>threshold_in_photons):
            ...
            if(secondCondition(threshold2,time_and_PD,photons_in_region,photonsPassed) == True):
                ...
                if(not recentTrig):            
                    ...
                    # This has been updated to check in a static vicinity of a SN -> probably will stick to 1.0 us
                    if(np.min(abs(time_of_event-eventsSN['eventTime'])) <1.0):# (integTime/2.0)):
    ...
    f = open(locationResults+'efficiencyandFakeTrig_ongoing_'+str(version)+'.csv', 'a+')
    f.write(str(threshold)+','+str(integTime)+','+str(threshold2)+','+str(eff)+','+str(fakeRate)+'\n')
    if(fakeRate):
    	g = open(locationResults+'fakeTriggers_'+str(version)+'.csv', 'a+')
    	g.write(str(threshold)+','+str(integTime)+','+str(threshold2)+','+str(eff)+','+str(fakeRate)+'\n')
    	for element in enumerate(fakeTrig):
    	    g.write(str(element)+', ')
    	g.write('\n\n\n')
    f.close()
